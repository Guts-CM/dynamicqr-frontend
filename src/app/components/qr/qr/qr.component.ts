import { NgTemplateOutlet } from '@angular/common';
import {
  ChangeDetectorRef,
  Component,
  DestroyRef,
  ElementRef,
  HostListener,
  NgZone,
  afterNextRender,
  computed,
  effect,
  inject,
  signal,
  untracked,
  viewChild,
} from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { FormBuilder, ReactiveFormsModule, Validators } from '@angular/forms';
import { switchMap } from 'rxjs';
import { MotionButtonDirective, MotionCardDirective, MotionInputDirective } from '../../../motion/motion.directives';
import { MotionService, killMotion, qs, qsa, type MotionTeardown } from '../../../motion/motion';
import { MOTION } from '../../../motion/motion-tokens';
import {
  animateList,
  animateModal,
  animateNavGlow,
  animateNavIndicator,
  animateSection,
  animateWorkspaceIn,
  animateWorkspaceOut,
  pulseElement,
  revealTargets,
} from '../../../motion/ui-motion';
import { AuthService } from '../../service/auth.service';
import {
  QR_TYPE_OPTIONS,
  formatQrCodeId,
  formatQrDay,
  formatQrTimestamp,
  formatUserName,
  isCssHex,
  isTransparentBackground,
  backgroundLabel,
  qrFileSlug,
  qrShapeLabel,
  qrTypeLabel,
} from '../qr-format';
import { QrResponse, parseQrFecha } from '../qr-response';
import { QrCreateComponent } from '../qr-create/qr-create.component';
import { QrCreatePayload, QrService } from '../qr.service';
import { QrStylePickerComponent, type QrStylePickerValue } from '../qr-style-picker/qr-style-picker.component';

type QrDialog = 'edit' | 'delete' | null;

type StyleSwatch = {
  label: string;
  value: string;
  color: string | null;
};

@Component({
  selector: 'app-qr',
  imports: [
    NgTemplateOutlet,
    ReactiveFormsModule,
    MotionButtonDirective,
    MotionCardDirective,
    MotionInputDirective,
    QrCreateComponent,
    QrStylePickerComponent,
  ],
  templateUrl: './qr.component.html',
  styleUrl: './qr.component.css',
  host: {
    class: 'block min-h-dvh',
    '[class.is-switching]': 'transitioning()',
  },
})
export class QrComponent {
  private readonly qrService = inject(QrService);
  private readonly authService = inject(AuthService);
  private readonly router = inject(Router);
  private readonly route = inject(ActivatedRoute);
  private readonly motion = inject(MotionService);
  private readonly destroyRef = inject(DestroyRef);
  private readonly host = inject(ElementRef<HTMLElement>);
  private readonly zone = inject(NgZone);
  private readonly cdr = inject(ChangeDetectorRef);
  private readonly fb = inject(FormBuilder);
  private readonly desktopNav = viewChild<ElementRef<HTMLElement>>('desktopNav');
  private readonly dockNav = viewChild<ElementRef<HTMLElement>>('dockNav');

  private observers: ResizeObserver[] = [];
  private glowStops: MotionTeardown[] = [];
  private navReady = false;
  private navFrame = 0;
  private navMove = 0;
  private navMovedAt = 0;

  private svgObjectUrl: string | null = null;
  private svgRequest = 0;
  private loadRequest = 0;
  private copiedTimer = 0;
  private dialogTimer = 0;
  private successTimer = 0;
  private viewGen = 0;
  private resumeId: number | null = null;

  readonly mode = signal<'manage' | 'create'>('manage');
  readonly transitioning = signal(false);
  protected readonly createBusy = signal(false);
  protected readonly createError = signal<string | null>(null);
  protected readonly createSuccess = signal(false);

  protected readonly loading = signal(true);
  protected readonly error = signal(false);
  protected readonly records = signal<QrResponse[]>([]);
  protected readonly selectedId = signal<number | null>(null);
  protected readonly mobileDetail = signal(false);
  protected readonly svgSrc = signal<string | null>(null);
  protected readonly svgLoading = signal(false);
  protected readonly svgError = signal(false);
  protected readonly downloadBusy = signal(false);
  protected readonly downloadError = signal<string | null>(null);
  protected readonly copied = signal(false);
  protected readonly dialog = signal<QrDialog>(null);
  protected readonly actionBusy = signal(false);
  protected readonly actionError = signal<string | null>(null);
  protected readonly transparentBg = signal(false);
  protected readonly editTipo = signal('url');
  protected readonly editStamp = signal(0);
  protected readonly typeOptions = QR_TYPE_OPTIONS;

  protected readonly editForm = this.fb.nonNullable.group({
    nombre: ['', Validators.required],
    tipo: ['url', Validators.required],
    destinoUrl: [''],
    contenido: [''],
    colorModulos: ['#111827'],
    colorFondo: ['#FFFFFF'],
    colorGradiente: ['#6366F1'],
    formaModulo: ['redondeado'],
    formaOjo: ['redondeado'],
  });

  protected readonly selected = computed(() => {
    const id = this.selectedId();
    return this.records().find((record) => record.qrId === id) ?? null;
  });

  protected readonly countLabel = computed(() => {
    const total = this.records().length;
    return total === 1 ? '1 código' : `${total} códigos`;
  });

  protected readonly editStyleValue = computed((): QrStylePickerValue => {
    this.editStamp();
    const value = this.editForm.getRawValue();
    return {
      colorModulos: value.colorModulos,
      colorFondo: value.colorFondo,
      colorGradiente: value.colorGradiente,
      formaModulo: value.formaModulo,
      formaOjo: value.formaOjo,
      transparent: this.transparentBg(),
    };
  });

  protected readonly navItems = [
    { id: 'panel', label: 'Panel' },
    { id: 'qr', label: 'QR' },
    { id: 'escaneos', label: 'Escaneos' },
    { id: 'versiones', label: 'Versiones' },
    { id: 'usuarios', label: 'Usuarios' },
  ] as const;

  protected readonly activeSection = signal<(typeof this.navItems)[number]['id']>('qr');
  protected readonly settledSection = signal<(typeof this.navItems)[number]['id']>('qr');
  protected readonly dateLabel = new Date().toLocaleDateString('es-MX', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  });

  constructor() {
    afterNextRender(() => {
      this.bootMotion();
      if (this.route.snapshot.queryParamMap.get('create') === '1') {
        this.openCreate();
      }
    });

    effect(() => {
      this.motion.reduced();
      untracked(() => {
        if (this.navReady) {
          this.setupNavGlow();
        }
      });
    });

    this.destroyRef.onDestroy(() => {
      cancelAnimationFrame(this.navFrame);
      this.observers.forEach((observer) => observer.disconnect());
      this.stopNavGlow();
      this.revokeSvg();
      window.clearTimeout(this.copiedTimer);
      window.clearTimeout(this.dialogTimer);
      window.clearTimeout(this.successTimer);
      killMotion(this.host.nativeElement);
    });
    this.editForm.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => {
      this.editStamp.update((stamp) => stamp + 1);
      this.editTipo.set(this.editForm.controls.tipo.value);
    });
    this.load();
  }

  protected creatingLocked(): boolean {
    return this.mode() === 'create' || this.transitioning();
  }

  protected startCreate(): void {
    this.openCreate();
  }

  protected selectSection(id: (typeof this.navItems)[number]['id']): void {
    if (id === 'escaneos' || id === 'qr') {
      return;
    }

    if (id === 'usuarios') {
      void this.router.navigateByUrl('/usuarios');
      return;
    }

    if (id === 'versiones') {
      void this.router.navigateByUrl('/versiones');
      return;
    }

    const url = id === 'panel' ? '/dashboard' : `/dashboard?section=${id}`;
    void this.router.navigateByUrl(url);
  }

  protected goDashboard(): void {
    void this.router.navigateByUrl('/dashboard');
  }

  protected signOut(): void {
    this.authService.logout();
    void this.router.navigateByUrl('/login');
  }

  @HostListener('document:keydown.escape')
  protected onEscape(): void {
    if (this.createBusy()) {
      return;
    }

    if (this.dialog() && !this.actionBusy()) {
      this.closeDialog();
      return;
    }

    if (this.mode() === 'create' && !this.createSuccess()) {
      this.cancelCreate();
    }
  }

  openCreate(): void {
    if (this.transitioning() || this.mode() === 'create' || this.actionBusy()) {
      return;
    }

    this.dialog.set(null);
    this.createError.set(null);
    this.createSuccess.set(false);
    this.resumeId = this.selectedId();
    this.swapView('create');
  }

  protected cancelCreate(): void {
    if (this.createBusy() || this.createSuccess() || this.mode() !== 'create') {
      return;
    }

    this.swapView('manage', () => this.resumeManagement(this.resumeId));
  }

  protected submitCreate(payload: QrCreatePayload): void {
    if (this.createBusy() || this.createSuccess() || this.transitioning()) {
      return;
    }

    this.createBusy.set(true);
    this.createError.set(null);

    this.qrService
      .create(payload)
      .pipe(
        switchMap(() => this.qrService.findAll()),
        takeUntilDestroyed(this.destroyRef),
      )
      .subscribe({
        next: (records) => {
          const sorted = this.sortRecords(records);
          this.records.set(sorted);
          this.createBusy.set(false);
          this.createSuccess.set(true);
          const created = this.pickCreated(sorted, payload.nombre);
          window.clearTimeout(this.successTimer);
          this.successTimer = window.setTimeout(
            () => this.returnToManagement(created?.qrId ?? null),
            MOTION.duration.section,
          );
        },
        error: () => {
          this.createBusy.set(false);
          this.createError.set('No se pudo crear el código QR.');
        },
      });
  }

  private returnToManagement(selectedId: number | null): void {
    if (this.mode() !== 'create') {
      return;
    }

    this.swapView('manage', () => {
      this.createSuccess.set(false);
      this.createError.set(null);
      this.resumeManagement(selectedId);
    });
  }

  private resumeManagement(selectedId: number | null): void {
    if (selectedId == null) {
      this.selectedId.set(null);
      this.mobileDetail.set(false);
      return;
    }

    const record = this.records().find((item) => item.qrId === selectedId) ?? null;
    if (!record) {
      return;
    }

    this.selectedId.set(selectedId);
    this.mobileDetail.set(true);
    this.fetchSvg(record);
  }

  private swapView(next: 'manage' | 'create', afterIn?: () => void): void {
    const gen = ++this.viewGen;
    this.transitioning.set(true);
    const root = this.workspaceRoot();

    const enter = (): void => {
      this.zone.run(() => {
        if (gen !== this.viewGen) {
          return;
        }

        this.mode.set(next);
        afterIn?.();
        this.cdr.detectChanges();
        this.playIncoming(gen);
      });
    };

    if (!root) {
      enter();
      return;
    }

    animateWorkspaceOut(root, enter);
  }

  private playIncoming(gen: number): void {
    const run = (): void => {
      this.zone.run(() => {
        if (gen !== this.viewGen) {
          return;
        }

        const incoming = this.workspaceRoot();
        const finish = (): void => {
          this.zone.run(() => {
            if (gen !== this.viewGen) {
              return;
            }

            this.revealWorkspace();
            this.transitioning.set(false);
          });
        };

        if (incoming) {
          animateWorkspaceIn(incoming, finish);
          return;
        }

        finish();
      });
    };

    this.zone.runOutsideAngular(() => {
      requestAnimationFrame(() => requestAnimationFrame(run));
    });
  }

  private revealWorkspace(): void {
    const host = this.host.nativeElement;
    const root = this.workspaceRoot() ?? host;
    revealTargets([host, root, ...qsa(root, '.card, .qr-item, [data-qr-motion]')]);
  }

  private workspaceRoot(): HTMLElement | null {
    return qs<HTMLElement>(
      this.host.nativeElement,
      this.mode() === 'create' ? 'app-qr-create' : '.qr-workspace',
    );
  }

  private pickCreated(records: QrResponse[], nombre: string): QrResponse | null {
    const matches = records.filter((record) => record.nombre?.trim() === nombre);
    return (matches.length ? matches : records)[0] ?? null;
  }

  protected load(): void {
    const request = ++this.loadRequest;
    this.loading.set(true);
    this.error.set(false);
    this.selectedId.set(null);
    this.mobileDetail.set(false);
    this.copied.set(false);
    this.clearSvg();

    this.qrService
      .findAll()
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (records) => {
          if (request !== this.loadRequest) {
            return;
          }

          this.records.set(this.sortRecords(records));
          this.loading.set(false);
          this.queueListMotion();
        },
        error: () => {
          if (request !== this.loadRequest) {
            return;
          }

          this.records.set([]);
          this.error.set(true);
          this.loading.set(false);
        },
      });
  }

  protected selectQr(record: QrResponse): void {
    if (record.qrId == null) {
      return;
    }

    const same = this.selectedId() === record.qrId;
    this.selectedId.set(record.qrId);
    this.mobileDetail.set(true);
    this.copied.set(false);
    this.downloadError.set(null);

    if (!same) {
      this.fetchSvg(record);
    }

    this.queueDetailMotion();
  }

  protected backToList(): void {
    this.mobileDetail.set(false);
  }

  protected codeId(record: QrResponse): string {
    return formatQrCodeId(record.qrId);
  }

  protected displayName(record: QrResponse): string {
    return record.nombre?.trim() || 'Sin nombre';
  }

  protected createdLabel(record: QrResponse): string {
    return formatQrDay(record.fechaCreacion);
  }

  protected typeLabel(record: QrResponse): string {
    return qrTypeLabel(record.tipo);
  }

  protected destination(record: QrResponse): string | null {
    const url = record.destinoUrl?.trim();
    return url || null;
  }

  protected destinationHref(record: QrResponse): string | null {
    const url = this.destination(record);
    if (!url) {
      return null;
    }

    return /^https?:\/\//i.test(url) ? url : `https://${url}`;
  }

  protected creatorLabel(record: QrResponse): string {
    return formatUserName(record.nombreCreador, record.usuarioCreador);
  }

  protected editorLabel(record: QrResponse): string {
    return formatUserName(record.nombreEditor, record.usuarioEditor);
  }

  protected createdAt(record: QrResponse): string {
    return formatQrTimestamp(record.fechaCreacion);
  }

  protected editedAt(record: QrResponse): string {
    return formatQrTimestamp(record.fechaEdicion);
  }

  protected moduleShape(record: QrResponse): string {
    return qrShapeLabel(record.estilo?.formaModulo);
  }

  protected eyeShape(record: QrResponse): string {
    return qrShapeLabel(record.estilo?.formaOjo);
  }

  protected swatches(record: QrResponse): StyleSwatch[] {
    const estilo = record.estilo;
    if (!estilo) {
      return [];
    }

    return [
      { label: 'Color de módulos', value: estilo.colorModulos?.trim() || '—', color: estilo.colorModulos },
      {
        label: 'Color de fondo',
        value: backgroundLabel(estilo.colorFondo),
        color: isTransparentBackground(estilo.colorFondo) ? null : estilo.colorFondo,
      },
      { label: 'Color de degradado', value: estilo.colorGradiente?.trim() || '—', color: estilo.colorGradiente },
    ];
  }

  protected swatchColor(color: string | null): string | null {
    const value = color?.trim() ?? '';
    return isCssHex(value) ? value : null;
  }

  protected previewBackground(record: QrResponse): string | null {
    if (isTransparentBackground(record.estilo?.colorFondo)) {
      return null;
    }

    return this.swatchColor(record.estilo?.colorFondo ?? null);
  }

  protected isClearBackground(record: QrResponse): boolean {
    return isTransparentBackground(record.estilo?.colorFondo);
  }

  protected hasStyle(record: QrResponse): boolean {
    return Boolean(record.estilo) || this.swatches(record).length > 0;
  }

  protected copyDestination(record: QrResponse, trigger: EventTarget | null): void {
    const url = this.destination(record);
    if (!url || typeof navigator === 'undefined' || !navigator.clipboard) {
      return;
    }

    void navigator.clipboard.writeText(url).then(() => {
      this.copied.set(true);
      pulseElement(trigger instanceof Element ? trigger : null);
      window.clearTimeout(this.copiedTimer);
      this.copiedTimer = window.setTimeout(() => this.copied.set(false), 1600);
    });
  }

  protected retrySvg(record: QrResponse): void {
    this.fetchSvg(record);
  }

  protected downloadQr(format: 'svg' | 'png'): void {
    const record = this.selected();
    if (!record || this.downloadBusy() || this.svgLoading()) {
      return;
    }

    const url = format === 'png' ? this.pngUrl(record) : record.urlSvg?.trim() || null;
    if (!url) {
      this.downloadError.set('No se pudo descargar el código QR.');
      return;
    }

    this.downloadBusy.set(true);
    this.downloadError.set(null);

    this.qrService
      .download(url)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (blob) => {
          this.saveBlob(blob, `${qrFileSlug(this.displayName(record))}.${format}`);
          this.downloadBusy.set(false);
        },
        error: () => {
          this.downloadBusy.set(false);
          this.downloadError.set('No se pudo descargar el código QR.');
        },
      });
  }

  protected onEditStyleChange(next: QrStylePickerValue): void {
    this.editForm.patchValue({
      colorModulos: next.colorModulos,
      colorFondo: next.colorFondo,
      colorGradiente: next.colorGradiente,
      formaModulo: next.formaModulo,
      formaOjo: next.formaOjo,
    });
    this.transparentBg.set(next.transparent);
  }

  protected openEdit(): void {
    const record = this.selected();
    if (!record || this.actionBusy()) {
      return;
    }

    this.patchEditForm(record);
    this.actionError.set(null);
    this.showDialog('edit');
  }

  protected openDelete(): void {
    if (!this.selected() || this.actionBusy()) {
      return;
    }

    this.actionError.set(null);
    this.showDialog('delete');
  }

  protected closeDialog(): void {
    if (this.actionBusy()) {
      return;
    }

    this.playDialog(false);
    window.clearTimeout(this.dialogTimer);
    this.dialogTimer = window.setTimeout(() => this.dialog.set(null), MOTION.duration.component);
  }

  protected saveEdit(): void {
    const id = this.selectedId();
    if (id == null || this.actionBusy()) {
      return;
    }

    this.editForm.markAllAsTouched();
    if (this.editForm.invalid) {
      return;
    }

    const value = this.editForm.getRawValue();
    const isUrl = value.tipo === 'url';
    if (isUrl && !/^https?:\/\/.+/i.test(value.destinoUrl.trim())) {
      this.actionError.set('La URL de destino debe comenzar con http:// o https://.');
      return;
    }

    if (!isUrl && !value.contenido.trim()) {
      this.actionError.set('El contenido es obligatorio para este tipo de código.');
      return;
    }

    this.actionBusy.set(true);
    this.actionError.set(null);

    this.qrService
      .update(id, {
        nombre: value.nombre.trim(),
        tipo: value.tipo,
        destinoUrl: isUrl ? value.destinoUrl.trim() : null,
        contenido: isUrl ? null : value.contenido.trim(),
        estilo: {
          colorModulos: value.colorModulos.trim() || null,
          colorFondo: this.transparentBg() ? 'transparent' : value.colorFondo.trim() || '#FFFFFF',
          colorGradiente: value.colorGradiente.trim() || null,
          formaModulo: value.formaModulo,
          formaOjo: value.formaOjo,
        },
      })
      .pipe(
        switchMap(() => this.qrService.findById(id)),
        takeUntilDestroyed(this.destroyRef),
      )
      .subscribe({
        next: (updated) => {
          this.replaceRecord(updated);
          this.fetchSvg(updated);
          this.actionBusy.set(false);
          this.closeDialog();
        },
        error: () => {
          this.actionBusy.set(false);
          this.actionError.set('No se pudieron guardar los cambios.');
        },
      });
  }

  protected confirmDelete(): void {
    const id = this.selectedId();
    if (id == null || this.actionBusy()) {
      return;
    }

    this.actionBusy.set(true);
    this.actionError.set(null);

    this.qrService
      .toggleActivo(id)
      .pipe(
        switchMap(() => this.qrService.findById(id)),
        takeUntilDestroyed(this.destroyRef),
      )
      .subscribe({
        next: (updated) => {
          this.replaceRecord(updated);
          this.actionBusy.set(false);
          this.closeDialog();
        },
        error: () => {
          this.actionBusy.set(false);
          this.actionError.set('No se pudo actualizar el estado del código.');
        },
      });
  }

  private patchEditForm(record: QrResponse): void {
    const tipo = (record.tipo || 'url').toLowerCase();
    this.editTipo.set(tipo);
    this.transparentBg.set(isTransparentBackground(record.estilo?.colorFondo));
    this.editForm.reset({
      nombre: record.nombre?.trim() || '',
      tipo,
      destinoUrl: record.destinoUrl?.trim() || '',
      contenido: record.contenido?.trim() || '',
      colorModulos: record.estilo?.colorModulos?.trim() || '#111827',
      colorFondo: isTransparentBackground(record.estilo?.colorFondo)
        ? '#FFFFFF'
        : record.estilo?.colorFondo?.trim() || '#FFFFFF',
      colorGradiente: record.estilo?.colorGradiente?.trim() || '#6366F1',
      formaModulo: this.normalizeShape(record.estilo?.formaModulo, 'redondeado'),
      formaOjo: this.normalizeShape(record.estilo?.formaOjo, 'redondeado'),
    });
  }

  private normalizeShape(value: string | null | undefined, fallback: string): string {
    const key = (value || fallback).trim().toLowerCase();
    const aliases: Record<string, string> = {
      rounded: 'redondeado',
      extraRounded: 'extra_redondeado',
      pill: 'extra_redondeado',
      soft: 'suave',
      square: 'cuadrado',
      circular: 'circulo',
      circle: 'circulo',
      diamond: 'diamante',
      rhombus: 'diamante',
      dots: 'circulo',
      dot: 'circulo',
    };

    return aliases[key] || key;
  }

  private replaceRecord(updated: QrResponse): void {
    this.records.update((records) =>
      this.sortRecords(records.map((record) => (record.qrId === updated.qrId ? updated : record))),
    );
  }

  private showDialog(kind: Exclude<QrDialog, null>): void {
    window.clearTimeout(this.dialogTimer);
    this.dialog.set(kind);
    const play = (attempt = 0): void => {
      const overlay = qs<HTMLElement>(this.host.nativeElement, '.qr-dialog-overlay');
      const panel = qs<HTMLElement>(this.host.nativeElement, '.qr-dialog-panel');
      if (!overlay || !panel) {
        if (attempt < 10) {
          requestAnimationFrame(() => play(attempt + 1));
        }
        return;
      }

      this.playDialog(true);
    };

    requestAnimationFrame(() => play());
  }

  private playDialog(open: boolean): void {
    const overlay = qs<HTMLElement>(this.host.nativeElement, '.qr-dialog-overlay');
    const panel = qs<HTMLElement>(this.host.nativeElement, '.qr-dialog-panel');
    if (!overlay || !panel) {
      return;
    }

    animateModal(overlay, panel, open);
  }

  private fetchSvg(record: QrResponse): void {
    const url = record.urlSvg?.trim();
    this.revokeSvg();
    this.svgSrc.set(null);
    this.svgError.set(false);

    if (!url) {
      this.svgLoading.set(false);
      this.svgError.set(true);
      return;
    }

    const request = ++this.svgRequest;
    this.svgLoading.set(true);

    this.qrService
      .loadSvg(url)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (objectUrl) => {
          if (request !== this.svgRequest) {
            URL.revokeObjectURL(objectUrl);
            return;
          }

          this.svgObjectUrl = objectUrl;
          this.svgSrc.set(objectUrl);
          this.svgLoading.set(false);
        },
        error: () => {
          if (request !== this.svgRequest) {
            return;
          }

          this.svgLoading.set(false);
          this.svgError.set(true);
        },
      });
  }

  private clearSvg(): void {
    this.svgRequest += 1;
    this.revokeSvg();
    this.svgSrc.set(null);
    this.svgLoading.set(false);
    this.svgError.set(false);
  }

  private revokeSvg(): void {
    if (this.svgObjectUrl) {
      URL.revokeObjectURL(this.svgObjectUrl);
      this.svgObjectUrl = null;
    }
  }

  private pngUrl(record: QrResponse): string | null {
    const png = record.urlPng?.trim();
    if (png) {
      return png;
    }

    const svg = record.urlSvg?.trim();
    if (!svg) {
      return null;
    }

    return svg.replace(/\/svg\/?$/i, '/png');
  }

  private saveBlob(blob: Blob, filename: string): void {
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = filename;
    link.rel = 'noopener';
    document.body.append(link);
    link.click();
    link.remove();
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
  }

  private sortRecords(records: QrResponse[]): QrResponse[] {
    return [...records].sort((left, right) => {
      const delta =
        (parseQrFecha(right.fechaCreacion)?.getTime() ?? 0) -
        (parseQrFecha(left.fechaCreacion)?.getTime() ?? 0);

      return delta || (right.qrId ?? 0) - (left.qrId ?? 0);
    });
  }

  private queueListMotion(): void {
    requestAnimationFrame(() => {
      const items = qsa<HTMLElement>(this.host.nativeElement, '.qr-item');
      animateList(items, { y: 8, stagger: 42, duration: 360 });
    });
  }

  private queueDetailMotion(): void {
    requestAnimationFrame(() => {
      const detail = qs<HTMLElement>(this.host.nativeElement, '.qr-detail-live');
      if (!detail) {
        return;
      }

      animateSection([
        { targets: qsa(detail, '[data-qr-motion]'), y: 10, duration: 420, stagger: 54, at: 0 },
      ]);
    });
  }

  private bootMotion(): void {
    this.setupNav();
    this.navReady = true;
    this.syncIndicators(true);
    this.setupNavGlow();
  }

  private setupNav(): void {
    const navs = this.navElements();
    if (typeof ResizeObserver === 'undefined') {
      return;
    }

    this.observers = navs.map((nav) => {
      const observer = new ResizeObserver(() => {
        if (performance.now() - this.navMovedAt < MOTION.duration.nav + 80) {
          return;
        }

        this.syncIndicators(true, this.navMove);
      });
      observer.observe(nav);
      return observer;
    });
  }

  private syncIndicators(instant = false, move = this.navMove): void {
    const section = this.activeSection();

    for (const nav of this.navElements()) {
      if (!nav.offsetWidth) {
        continue;
      }

      const indicator = qs<HTMLElement>(nav, '.nav-indicator');
      const active = qs<HTMLElement>(nav, `.nav-link[data-section="${section}"]`);
      if (!indicator || !active) {
        continue;
      }

      animateNavIndicator(indicator, active, nav, instant, () => {
        if (this.navMove === move) {
          this.settledSection.set(section);
        }
      });
      nav.classList.add('has-indicator');
    }
  }

  private setupNavGlow(): void {
    this.stopNavGlow();
    this.glowStops = this.navElements()
      .map((nav) => qs<HTMLElement>(nav, '.nav-indicator'))
      .filter((indicator): indicator is HTMLElement => Boolean(indicator))
      .map((indicator) =>
        animateNavGlow(indicator, {
          enabled: () => this.motion.enabled(),
        }),
      );
  }

  private stopNavGlow(): void {
    this.glowStops.forEach((stop) => stop());
    this.glowStops = [];
  }

  private navElements(): HTMLElement[] {
    return [this.desktopNav()?.nativeElement, this.dockNav()?.nativeElement].filter(
      (node): node is HTMLElement => Boolean(node),
    );
  }
}
